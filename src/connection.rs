...
impl Connection {
        ...
        loop {
            ...
            match packet_type {
                2 => {
                    ...
                    // Negotiate protocol-level capabilities from the ACCEPT flags
                    // (END_OF_RESPONSE markers, fast auth, OOB). Without this the
                    // END_OF_RESPONSE capability bit is never set in the compile
                    // caps sent to the server, so multi-packet query responses
                    // carry no terminator and the multi-packet reader would hang.
    ...
    async fn execute_query_with_params(&self, statement: &Statement, params: &[Value]) -> Result<QueryResult> {
        let prefetch_rows = 1_000_000; // PATCH: fetch whole result in one execute (fetch_more continuation is broken upstream, issue #9)
        ...
        // Receive and parse response. PATCH: a query result larger than one TNS
        // packet (roughly one SDU of rows) spans multiple packets; `receive()`
        // reads only the first, leaving the parser to run off the end (buffer
        // underflow). `receive_response()` accumulates every packet until the
        // END_OF_RESPONSE flag, so the whole result is parsed.
        ...
        if has_lob_columns && !statement.requires_define() {
            ...
            // Receive the re-execute response (PATCH: multi-packet safe)
